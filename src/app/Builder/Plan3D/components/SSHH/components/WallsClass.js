...
export default class WallsClass {
	...
	setMeasures({ lavamanos, pasillo, inodoro, pasillo_de_entrada, ancho_de_cubiculo }, baths_amount, wall_thickness) {
		...
		this.back = {
			length: (this.front.length * 2) + pasillo_de_entrada - 0.200006105005, // 6.4 m | 320 u      // + pasillo_de_entrada es por el + 1 metro de puerta (entrada). 
	...
	setPosition({ pasillo_de_entrada }, wall_thickness) {
		this.front.position = {
			1: [-this.front.length - pasillo_de_entrada - wall_thickness, 0, 0], // FRONT LEFT
			2: [-wall_thickness - 0.200006105005, 0, 0], // FRONT RIGHT
		}

		this.sides.position = {
			1: [-pasillo_de_entrada - this.front.length - wall_thickness + 8, 0, 0], // SIDE LEFT
			2: [this.front.length, 0, 0] // SIDE RIGHT
			// 1: [-pasillo_de_entrada - this.front.length - wall_thickness - 200, 0, 0], // SIDE LEFT
			// 2: [this.front.length + 200, 0, 0] // SIDE RIGHT
		}

		this.back.position = {
			1: [-wall_thickness - pasillo_de_entrada - this.front.length, 0, -this.sides.length - wall_thickness]
		}
	}
